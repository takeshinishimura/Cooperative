...
[https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290289](https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290289){target="_blank"}
:::
...
最後のところでエラーが出る場合は，`p` だけにして，プラス以降を入力しない場合どうなるか確かめてください。
...
p <- ggplot(data = df_long, aes(x = 年度, y = 組合数, group = 消費生活協同組合, colour = 消費生活協同組合)) +
  geom_line() +
  geom_point() +
...
```{r}
#| warning: false
ggsave(filename = file.path(outdir, "number_of_coop.pdf"), plot = p, width = 8, height = 4)
```
あるいは，次のコードを実行します。
```{r}
#| warning: false
ggsave(filename = file.path(outdir, "number_of_coop.png"), plot = p, width = 8, height = 4)
```
...
ggsave(filename = file.path(outdir, "number_of_coop_members.pdf"), plot = p, width = 8, height = 4)
```
...
[https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290290](https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290290){target="_blank"}
:::
...
ggsave(filename = file.path(outdir, "number_of_coop_types.pdf"), plot = p, width = 8, height = 4)
```
