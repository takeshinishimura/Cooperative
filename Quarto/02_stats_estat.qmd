...
上記のデータフレームから折れ線グラフを作成する方法は，@sec-simpleplot の方法と同じである。
...
```{r}
#| warning: false
ggsave(g, file = file.path(outdir, "number_of_ja_gg.pdf"), width = 10, height = 7)
ggsave(g, file = file.path(outdir, "number_of_ja_gg.png"), width = 10, height = 7)
```
