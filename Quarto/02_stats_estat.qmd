...
## 消費生活協同組合（連合会）実態調査 / 令和６年度消費生活協同組合（連合会）実態調査
2-1 組合数及び組員員数の状況

<https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290289>{target="_blank"}
:::
...
次のコマンドで，自分がいまどこで作業をしているのか（作業ディレクトリの場所）を確認できます。
```{r}
#| eval: false
getwd()
```
...
これは関数と呼ばれるもので，既に決まっています。
通常，頻繁に使う関数は覚えておくべきであり，覚えるためにはその関数の意味と関数の綴りをあなたの頭の中で対応させておくとよいでしょう。

よくわからないと思うので，最初は，先ほどダウンロードしたExcelファイルを現在の作業ディレクトリに移動して，そこで作業するとよいでしょう。
ファイルの移動は，Rではなく，Windowsならエクスプローラー，macOSならFinderを使います。

### Excelファイルの読み込み

readxlパッケージを初めて使う場合のみ，以下のコマンドを実行します。
一度実行すれば，パソコンの電源を切ったとしても，再度実行する必要はありません（何度実行してもエラーにはなりませんが，2回目以降は実行する意味がありません）。
```{r}
#| eval: false
...
先ほど，e-StatからダウンロードしたファイルをRに読み込みます。
...
```{r}
#| results: 'hide'
pdf(file.path(outdir, "number_of_coop_members_plot.pdf"), width = 10, height = 10, family = "Japan1GothicBBB")
...
df_long <- reshape2::melt(
  df[, c("年度", grep("組合数（", names(df), value = TRUE))],
  id.vars = "年度"
)
names(df_long)[2:3] <- c("消費生活協同組合", "組合数")

p <- ggplot(data = df_long) +
  geom_line(aes(x = 年度, y = 組合数, colour = 消費生活協同組合)) +
  geom_point(aes(x = 年度, y = 組合数, colour = 消費生活協同組合)) +
...
最後のコードの `family = "Hiragino Sans"` の部分は，あなたのパソコンにインストールされているフォントに依存します。
Windows の場合，`+ theme(text = element_text(family = "Hiragino Sans"))`の部分を削除してください。
...
p <- ggplot(data = df_long, aes(x = 年度, y = 組合数, colour = 消費生活協同組合)) +
  geom_line() +
  geom_point() +
  ...
  labs(
    title = "消費生活協同組合の数の推移",
    caption = "出所：厚生労働省『令和6年度消費生活協同組合（連合会）実態調査』\n（https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290289）"
  ) +
  theme_bw() +
  theme(text = element_text(family = "Hiragino Sans"))
...
```{r}
#| warning: false
ggsave(filename = "number_of_coop.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
あるいは，次のコードを実行します。
```{r}
#| warning: false
ggsave(filename = "number_of_coop.png", plot = p, path = outdir, width = 8, height = 4)
```
図をWordに貼り付けたい場合は，WindowsだとPNGで書き出し，macOSだとPNGまたはPDFで書き出すのがよいでしょう。

同じようにして，組合員数の推移のグラフを描くことができます。
```{r}
#| warning: false
df_long <- reshape2::melt(
  df[, c("年度", grep("組合員数（", names(df), value = TRUE))],
  id.vars = "年度"
)
names(df_long)[2:3] <- c("消費生活協同組合", "組合員数")

p <- ggplot(data = df_long, aes(x = 年度, y = 組合員数 / 10000, colour = 消費生活協同組合)) +
  geom_line() +
  geom_point() +
...
ggsave(filename = "number_of_coop_members.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
...
::: {.callout-note collapse="false" icon="true"}
## 消費生活協同組合（連合会）実態調査 / 令和6年度消費生活協同組合（連合会）実態調査
2-2 事業種類別組合数の状況

<https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290290>{target="_blank"}
:::
...
p <- ggplot(data = df_long, aes(x = 年度, y = 組合数, colour = 消費生活協同組合)) +
  ...
  labs(
    ...
    caption = "出所：厚生労働省『令和6年度消費生活協同組合（連合会）実態調査』\n（https://www.e-stat.go.jp/stat-search/files?stat_infid=000040290290）"
  ) +
  theme_bw() +
  theme(text = element_text(family = "Hiragino Sans"))

ggsave(filename = "number_of_coop_types.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
