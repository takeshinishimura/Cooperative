...
g + theme(text = element_text(family = "HiraKakuProN-W3"))
ggsave(g, file = file.path(outdir, "benefits_by_division_for_each_year_all.pdf"), width = 8, height = 4)
...
g <- ggplot(data = df_all2, aes(x = `時間軸（年次）`, y = value, group = `損益計算書（総括）`, colour = `損益計算書（総括）`)) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.title = element_blank())
g + theme(text = element_text(family = "HiraKakuProN-W3"))
ggsave(g, file = file.path(outdir, "revenues_by_division_for_each_year_all.pdf"), width = 8, height = 4)
...
g <- ggplot(data = df_all2, aes(x = `時間軸（年次）`, y = value, group = `損益計算書（総括）`, colour = `損益計算書（総括）`)) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.title = element_blank())
g + theme(text = element_text(family = "HiraKakuProN-W3"))
ggsave(g, file = file.path(outdir, "costs_by_division_for_each_year_all.pdf"), width = 8, height = 4)
```
...
df_pref <- df[df$`都道府県` != "全国", ]

df_pref2 <- df_pref[grep("信用事業総利益", df_pref$`損益計算書（総括）`), ]

g <- ggplot(data = df_pref2, aes(x = `時間軸（年次）`, y = value, group = `都道府県`, colour = `都道府県`)) +
  geom_line() +
  geom_point() +
  gghighlight(都道府県 == "愛媛県" | 都道府県 == "北海道" | 都道府県 == "愛知県") +
  scale_y_continuous(labels = scales::comma_format(), name = "（千円）") +
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.title = element_blank())
g + theme(text = element_text(family = "HiraKakuProN-W3"))
ggsave(g, file = file.path(outdir, "benefits_by_division_for_each_year_pref_shinyou.pdf"), width = 8, height = 4)

df_pref2 <- df_pref[grep("共済事業総利益", df_pref$`損益計算書（総括）`), ]
...
g <- ggplot(data = df_pref2, aes(x = `時間軸（年次）`, y = value, group = `都道府県`, colour = `都道府県`)) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.title = element_blank())
g + theme(text = element_text(family = "HiraKakuProN-W3"))
ggsave(g, file = file.path(outdir, "benefits_by_division_for_each_year_pref_kyousai.pdf"), width = 8, height = 4)

df_pref2 <- df_pref[grep("購買事業総利益", df_pref$`損益計算書（総括）`), ]
...
ggsave(g, file = file.path(outdir, "benefits_by_division_for_each_year_pref_koubai.pdf"), width = 8, height = 4)

df_pref2 <- df_pref[grep("販売事業総利益", df_pref$`損益計算書（総括）`), ]
...
ggsave(g, file = file.path(outdir, "benefits_by_division_for_each_year_pref_hanbai.pdf"), width = 8, height = 4)
```
