...
<https://www.e-stat.go.jp/dbview?sid=0003406360>{target="_blank"}
:::

::: {.callout-note collapse="false" icon="true"}
## 農業協同組合及び同連合会一斉調査 / 長期累年

<https://www.e-stat.go.jp/stat-search/database?statdisp_id=0003410051>{target="_blank"}
:::
...
<https://www.e-stat.go.jp/stat-search/database?statdisp_id=0002110844>{target="_blank"}
:::
...
<https://www.e-stat.go.jp/stat-search/database?statdisp_id=0002063186>{target="_blank"}
:::
...
<https://www.e-stat.go.jp/stat-search/database?statdisp_id=0001998576>{target="_blank"}
:::
...
<https://www.e-stat.go.jp/stat-search/database?statdisp_id=0002015583>{target="_blank"}
:::
...
new_df$都道府県 <- ifelse(
  new_df$都道府県 %in% c("全国", "北海道", "東京", "京都", "大阪"),
  paste0(new_df$都道府県, c("", "", "都", "府", "府")[match(new_df$都道府県, c("全国", "北海道", "東京", "京都", "大阪"))]),
  paste0(new_df$都道府県, "県")
)
...
rdf2019 <- df2019 |>
  rename(組合 = `(r001-01-1-004)組合員数及び組合員戸数`) |>
  filter(組合 %in% c("正組合員_個人_うち女性")) |>
  mutate(組合 = sub("_個人_うち女性", "", 組合)) |>
  select(都道府県, 年, 組合, value)

rdf2020 <- df2020 |>
  rename(組合 = `(R001-02-1-004)組合員数及び組合員戸数`) |>
  filter(組合 %in% c("正組合員_個人_女性")) |>
  mutate(
    組合 = sub("_個人_女性", "", 組合)
  ) |>
  select(都道府県, 年, 組合, value)
...
rdf2021 <- df2021 |>
  rename(組合 = `(NOUKYOU1-21-1-004)組合員数及び組合員戸数`) |>
  filter(組合 %in% c("一戸複数正組合員制実施組合数_正組合員_個人_女性")) |>
  mutate(
    組合 = gsub("一戸複数正組合員制実施組合数_|_個人_女性", "", 組合)
  ) |>
  select(都道府県, 年, 組合, value)
...
rdf2022 <- df2022 |>
  rename(組合 = `(NOUKYOU1-22-1-004)組合員数及び組合員戸数`) |>
  filter(組合 %in% c("正組合員_個人_女性")) |>
  mutate(組合 = sub("_個人_女性", "", 組合)) |>
  select(都道府県, 年, 組合, value)

new_df_female <- bind_rows(
  rdf2019,
  rdf2020,
  rdf2021,
  rdf2022
...
new_df_female$都道府県 <- ifelse(
  new_df_female$都道府県 %in% c("全国", "北海道", "東京", "京都", "大阪"),
  paste0(new_df_female$都道府県, c("", "", "都", "府", "府")[match(new_df_female$都道府県, c("全国", "北海道", "東京", "京都", "大阪"))]),
  paste0(new_df_female$都道府県, "県")
)
...
```{r}
rdf2 <- df2 |>
  filter(grepl("戸数", `個人・団体`)) |>
  select(都道府県, 年, 組合, value)

rdf2019 <- df2019 |>
  rename(組合 = `(r001-01-1-004)組合員数及び組合員戸数`) |>
  filter(組合 %in% c("正組合員戸数", "准組合員戸数")) |>
  select(都道府県, 年, 組合, value)
...
rdf2021 <- df2021 |>
  rename(組合 = `(NOUKYOU1-21-1-004)組合員数及び組合員戸数`) |>
  filter(組合 %in% c("一戸複数正組合員制実施組合数_正組合員戸数")) |>
  mutate(
    組合 = gsub("一戸複数正組合員制実施組合数_", "", 組合)
  ) |>
  select(都道府県, 年, 組合, value)
...
new_df_household$都道府県 <- ifelse(
  new_df_household$都道府県 %in% c("全国", "北海道", "東京", "京都", "大阪"),
  paste0(new_df_household$都道府県, c("", "", "都", "府", "府")[match(new_df_household$都道府県, c("全国", "北海道", "東京", "京都", "大阪"))]),
  paste0(new_df_household$都道府県, "県")
)
...
p <- ggplot(
    data = df_JAmembers |> filter(組合 == "合計"),
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
ggsave(file = "ja_members_1993-2022.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
...
#| include: false
#| warning: false
ggsave(file = "ja_members_sei_vs_jun_1993-2022.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
...
p <- ggplot(
    data = new_df |> filter(都道府県 != "全国", 組合 == "合計"),
    aes(x = 年, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
  gghighlight(
...
#| include: false
#| warning: false
ggsave(file = "ja_members_sei_vs_jun_by_pref.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
p <- ggplot(
    data = new_df |> filter(都道府県 == "愛媛県", 組合 != "合計"),
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
#| include: false
#| warning: false
ggsave(file = "ja_members_sei_vs_jun_Ehime.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
p <- ggplot(
    data = new_df |> filter(都道府県 == "北海道", 組合 != "合計"),
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
#| include: false
#| warning: false
ggsave(file = "ja_members_sei_vs_jun_Hokkaido.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
p <- ggplot(
    data = new_df |> filter(都道府県 == "愛知県", 組合 != "合計"),
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
#| include: false
#| warning: false
ggsave(file = "ja_members_sei_vs_jun_Aichi.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
p <- ggplot(
    data = new_df |> filter(都道府県 == "京都府", 組合 != "合計"),
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
#| include: false
#| warning: false
ggsave(file = "ja_members_sei_vs_jun_Kyoto.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
p <- ggplot(
    data = new_df |> filter(都道府県 == "徳島県", 組合 != "合計"),
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
#| include: false
#| warning: false
ggsave(file = "ja_members_sei_vs_jun_Tokushima.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
...
```{r}
df_JAmembers_female <- new_df_female |>
  filter(都道府県 == "全国")
df_JAmembers_female

p <- ggplot(
    data = df_JAmembers_female,
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
#| include: false
#| warning: false
ggsave(file = "ja_members_female.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
...
#| include: false
#| warning: false
ggsave(file = "ja_members_female_with_all.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

組合員戸数の推移はどうでしょうか。

```{r}
df_JAmembers_household <- new_df_household |>
  filter(都道府県 == "全国")
df_JAmembers_household

p <- ggplot(
    data = df_JAmembers_household,
    aes(x = 年, y = value, colour = 組合)
  ) +
  geom_line() +
  geom_point() +
...
#| include: false
#| warning: false
ggsave(file = "ja_members_household.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
