...
ダイアグラム作成に，そこまで多様な機能は求めていない。
また，Adobe IllustratorもOmniGraffleもいずれも図の作成において有益ではあるが，ダイヤグラム時にデザインを決めなければならないことが個人的には面倒だと感じる。
また，いずれのソフトウェアも構造を理解しながらダイアグラムを描くには不向きである
...
似たようなツールとして，[Mermaid](https://mermaid.js.org/){target="_blank"}がある。
...
macOSの場合，Terminalに以下のようなコマンドを打ち込むことで，[Homebrew](https://brew.sh/){target="_blank"}を使ってインストールできる。

```
$ brew install graphviz
```

Windowsの場合は，以下から最新の64-bit版をダウンロードして，インストールすればよい。
...
これでうまくいくはずであるが，なぜかこのやり方ではエラーが出てしまい，解決策がわからない。
