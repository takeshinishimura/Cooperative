...
<https://www.e-stat.go.jp/stat-search/files?stat_infid=000040060822>{target="_blank"}

<https://www.e-stat.go.jp/stat-search/files?stat_infid=000040060823>{target="_blank"}

<https://www.e-stat.go.jp/stat-search/files?stat_infid=000040060824>{target="_blank"}
:::
...
ggsave(file = "number_of_ja_by_pref_lollipop.pdf", plot = p, device = cairo_pdf, path = outdir, width = 10, height = 7)
```
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_lollipop_ordered.pdf", plot = p, device = cairo_pdf, path = outdir, width = 10, height = 7)
```
...
色が偏っているので，四分位数に基づいて色分けしてみることにします。
```{r}
df_pref <- data.frame(
  pref = df_sougou$pref,
...
```{r}
#| include: false
ggsave(file = "number_of_ja_map_by_pref.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 7)
```
...
```{r}
#| include: false
ggsave(file = "number_of_ja_map_by_pref_bertin.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 7)
```
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_map2.pdf", plot = p, device = cairo_pdf, path = outdir, width = 10, height = 10)
```
...
df2021 <- df2021 |>
  filter(
    `(R006-02-2-002)都道府県` %in% pref,
    grepl("（参考１）出資組合と非出資組合の計（専門農協、業種別）_", `(R006-03-1-001)総括表`),
...
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[1]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
  gghighlight(
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_ippannoukyou.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
#| warning: false
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[2]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_chikusan.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
#| warning: false
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[3]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_rakunou.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
#| warning: false
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[4]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_youkei.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
#| warning: false
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[5]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_makinokanri.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
#| warning: false
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[6]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_engeitokusan.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
#| warning: false
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[7]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_nousonkougyou.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
...
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[8]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_noujihousou.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```

```{r}
#| warning: false
p <- ggplot(
    data = df_senmon2 |> filter(区分 == levels(区分)[9]),
    aes(x = year, y = value, colour = 都道府県)
  ) +
  geom_line() +
  geom_point() +
...
```{r}
#| include: false
ggsave(file = "number_of_ja_by_pref_senmon_sonota.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
