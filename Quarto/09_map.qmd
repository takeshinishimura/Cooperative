...
その前に，ggplotでは日本語の変数名では処理できないようなので，変数名をアスキー文字に変換しておく。
...
ggplotの前に，変数名をアスキー文字に変換するなど，先ほどと同じ前処理が必要である。
...
また，経度と緯度を示す線など，見た目はggplotの書き方次第で如何様にもなる。
...
```{r}
#| include: false
ggsave(g, file = file.path(outdir, "number_of_ja_map2.pdf"), width = 14, height = 14)
```
