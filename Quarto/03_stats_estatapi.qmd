...
## 農業協同組合等現在数統計 / 確報 令和６年度農業協同組合等現在数統計
1 総括の部 農業協同組合、同連合会及び農事組合法人数の推移（第１表）

<https://www.e-stat.go.jp/stat-search/files?stat_infid=000040282295>{target="_blank"}
:::

上述のサイトから，`r006-02-001.xlsx` という名前のExcel形式のファイルをダウンロードできます。
...
#| eval: false
library(readxl)
df <- readxl::read_excel("r006-02-001.xlsx", col_names = FALSE, skip = 12)
```
```{r}
#| echo: false
#| message: false
library(readxl)
df <- readxl::read_excel("../data/r006-02-001.xlsx", col_names = FALSE, skip = 12)
```
...
df$年次 <- sub("\\..*?$", "", df$年次)
df$年次 <- sub("^S", "昭和", df$年次)
df$年次 <- sub("^H", "平成", df$年次)
df$年次 <- sub("^R", "令和", df$年次)
...
plot(df[, c("年次", "単位農協（総合農協）")], type = "o", main = "総合農協数の推移", xlab = "各年3月末", pch = 16, las = 1)
```
...
matplot(x = df$年次, y = df[, c("単位農協（総合農協）", "単位農協（専門農協（出資組合））", "単位農協（専門農協（非出資組合））")], type = "o", main = "単位農協数の推移", xlab = "各年3月末", ylab = "農業協同組合数", col = 2:4, lwd = 1, lty = 1, pch = 16, las = 1)
...
`statsDataId = "0002122464"` にある数字は，e-Statの[農業協同組合等現在数統計 確報 令和５年度農業協同組合等現在数統計 総括の部 1 農業協同組合、同連合会及び農事組合法人数の推移（第１表）](https://www.e-stat.go.jp/dbview?sid=0002122464){target="_blank"}のページを指すIDです。
なぜか，令和6年度のデータはありません。
...
response <- httr::GET(
  ...
  query = list(
    ...
    statsDataId = "0002122464",
    sectionHeaderFlg = 2
  )
)
response <- httr::content(response)
# df <- read.csv(text = sub("^.*\\\"VALUE\\\"\n", "", response), header = TRUE, check.names = FALSE)
df <- readr::read_csv(sub("^.*\\\"VALUE\\\"\n", "", response))

names(df)[2] <- "時点"
names(df)[4] <- "区分"

# 年次の整理
df <- df[df$時点 != "1996年3月末(変更前)", ]
df <- df[df$時点 != "2024年4月1日", ]
df$年次 <- as.integer(gsub("年3月末.*$", "", df$時点))

# 区分の整理
df$区分 <- gsub("_", "（", df$区分)
df$区分 <- unlist(lapply(df$区分, function(x) {
...
df2 <- df[grepl("^単位農協", df$区分), ]

p <- ggplot(data = df2, aes(x = 年次, y = value, colour = 区分)) +
  geom_line() +
  geom_point() +
  ...
  ggtitle("単位農協数の推移") +
  labs(y = "農業協同組合数",
       caption = "出所：農林水産省『令和5年度農業協同組合等現在数統計』\n（https://www.e-stat.go.jp/dbview?sid=0002122464）") +
  theme_bw() +
...
最後のところでエラーが出る場合は，`p` だけにして，プラス以降を入力しないでください。
...
ggsave(filename = "number_of_ja.pdf", plot = p, device = cairo_pdf, path = outdir, width = 8, height = 4)
```
```{r}
#| include: false
outdir <- "../fig"
...
ここで示した方法は，[Rからe-Stat APIを使う - Qiita](https://qiita.com/nozma/items/f88f5cc60ab63461deae){target="_blank"} が参考になりました。
これ以外にも，e-Statからデータを取得する方法はあります。
例えば，[estatapi](https://yutannihilation.github.io/estatapi/){target="_blank"} パッケージを使うと，ほぼ同じことがもっと簡単にできるようになります。
興味ある人は，自分で調べてください。
