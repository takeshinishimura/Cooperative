...
[https://www.e-stat.go.jp/stat-search/files?stat_infid=000040282295](https://www.e-stat.go.jp/stat-search/files?stat_infid=000040282295){target="_blank"}
:::
...
```{r}
df |> data.frame(check.names = FALSE) |> head()
str(df)
```
...
```{r}
plot(df[, c("年次", "単位農協（総合農協）")], type = "o", main = "総合農協数の推移", xlab = "年3月末", pch = 16, las = 1)
```
...
rownames(dfcoop) <- dfcoop[[1]]
dfcoop <- dfcoop[, -1]
matplot(dfcoop[, c("単位農協（総合農協）", "単位農協（専門農協（出資組合））", "単位農協（専門農協（非出資組合））")], type = "o", main = "単位農協数の推移", xlab = "年3月末", ylab = "農業協同組合数", col = 2:4, lwd = 1, lty = 1, pch = 16, las = 1,  axes = FALSE)
...
ggsave(filename = file.path(outdir, "number_of_ja.pdf"), plot = p, width = 8, height = 4)
```
```{r}
#| include: false
outdir <- "../fig"
