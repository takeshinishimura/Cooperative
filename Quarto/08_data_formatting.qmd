...
`data.frame()` で指定する `check.names =` はこれに関係する引数である。
...
ただし，こういった書き方をするくらいであれば，次の表記に統一した方がスマートではないだろうか。
```{r}
#| eval: false
df[, "時間軸（年度次）"]
```
...
ggsave(g, file = "number_of_ja_members.pdf", path = outdir, width = 8, height = 4)
```
...
#| include: false
#| warning: false
ggsave(g, file = "number_of_ja_members_sei_vs_jun.pdf", path = outdir, width = 8, height = 4)
```
...
ggsave(g, file = "number_of_ja_members_kojin_dantai.pdf", path = outdir, width = 8, height = 4)
```
...
ggsave(g, file = "number_of_ja_members_female.pdf", path = outdir, width = 8, height = 4)
```
...
ggsave(g, file = "number_of_ja_members_female_with_all.pdf", path = outdir, width = 8, height = 4)
```
...
ggsave(g, file = "number_of_ja_members_female_ratio.pdf", path = outdir, width = 8, height = 4)
```
...
ggsave(g, file = "number_of_ja_members_household.pdf", path = outdir, width = 8, height = 4)
```
...
ggsave(g, file = "number_of_ja_multiple_members_per_household.pdf", path = outdir, width = 8, height = 4)
```
...
ggsave(g, file = "number_of_ja_members_nonfarmer.pdf", path = outdir, width = 8, height = 4)
```
